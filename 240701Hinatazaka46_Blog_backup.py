...
def downloadImg(article, name, saveDir):
    ...
    for i, img in enumerate(img_list):
        try:
            src = img['src']
            s = "<" + "img src=" + '"' + str(i-cnt) + '.jpg"' +">"
            img_list[i].replace_with(s)
            if src != '':
                try:
                    tmp = requests.get(src)
                    with open(saveDir + str(i-cnt) + '.jpg', 'wb') as f:
                        f.write(tmp.content)
...
#"潮紗理菜": 2, "影山優佳": 4,"四期生リレー":2000 , "宮田愛萌": 19, "渡邉美穂":, 20 "齊藤京子": 6, "岸帆夏": 26
member = { "加藤史帆": 5, "佐々木久美": 7, "佐々木美玲": 8, "高瀬愛奈": 9, "高本彩花": 10
          , "東村芽依": 11, "金村美玖": 12, "河田陽菜": 13, "小坂菜緒": 14, "富田鈴花": 15, "丹生明里": 16, "濱岸ひより": 17, "松田好花": 18
          , "上村ひなの": 21, "髙橋未来虹": 22, "森本茉莉": 23, "山口陽世": 24,"ポカ": "000"
          , "石塚瑶季": 25, "小西夏菜実": 27, "清水理央": 28, "正源司陽子": 29, "竹内希来里": 30
